...
<cfif url.action EQ "Add">
   ...
   <cfif Url.Type EQ "M">
      <cfif Not IsDefined("form.MbrID")>
	    <cflocation url="AddMember.cfm?Type=#Url.Type#&GroupID=#form.GroupID#&e=1" addtoken="No">
	  </cfif>
	  
	  <cfif Not IsDefined("form.GroupID")>
	  ...
	  <cfif IsDefined("Form.ISAdmin") AND  Not IsDefined("Form.UserNameExists")>
	    <cfif Len(Trim(form.username)) EQ 0>
		  <cflocation url="AddMember.cfm?Type=#Url.Type#&GroupID=#form.GroupID#&e=2" addtoken="No">
		</cfif>
	  ...
	  <cfquery name="AddNewMbr" datasource="#Application.dsn#">
		...
		Values(#Form.GroupID#, #Form.MbrID#, <cfif IsDefined("form.IsPrimary")>1,<cfelse>0,</cfif> <cfif IsDefined("form.IsAdmin")>1,<cfelse>0,</cfif> <cfif IsDefined("form.IsPrimary")>1<cfelse>#NextRank#</cfif>)
	  </cfquery>
     ...
     <cfif Len(Trim(Form.firstname)) EQ 0>
	  ...
	  <cfif IsDefined("form.IsAdmin")>
	   ...
	   <cfquery name="UpdMember" datasource="#Application.dsn#">
	      Update Members 
		    Set StaffID = <cfif Isdefined("Form.IsStaff")>99<cfelse>Null</cfif>,
		 		Title = <cfif Len(Trim(form.Title)) GT 0>'#Form.Title#',<cfelse>Null,</cfif>
	   ...
	   <cfquery name="UpdMember" datasource="#Application.dsn#">
	      Update GrpMembers
		    Set IsPrimary = <cfif IsDefined("ISPrimary")>1<cfelse>0</cfif>,
			    IsAdmin = <cfif IsDefined("ISAdmin")>1<cfelse>0</cfif>,
